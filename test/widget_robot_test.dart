...
void main() {
  group('TaskList Widget Tests with TaskManager', () {
    group('Basic Operations', () {
      testWidgets('タスク追加・削除ができる', (tester) async {
        await tester.pumpMyApp(useController: false);
    ...
    group('Reorder Operations', () {
      testWidgets('タスクを並び替えできる', (tester) async {
        await tester.pumpMyApp(useController: false);
        final robot = TaskListRobot(tester);
      ...
      testWidgets('タスクの並び替えの様々なパターン', (tester) async {
        await tester.pumpMyApp(useController: false);
        final robot = TaskListRobot(tester);
      ...
      testWidgets('タスクの並び替え後に他の操作が正しく動作する', (tester) async {
        await tester.pumpMyApp(useController: false);
        final robot = TaskListRobot(tester);
      ...
      testWidgets('タスクの並び替えをキャンセルできる', (tester) async {
        await tester.pumpMyApp(useController: false);
        final robot = TaskListRobot(tester);
      ...
      testWidgets('大量のタスクでも並び替えが正常に動作する', (tester) async {
        ...
        // 初期状態確認（最初の5件）
        await robot.expectPartialTaskOrder([
          'Task 1',
          'Task 2',
          'Task 3',
          'Task 4',
        ...
        // より確実な並び替え操作
        await tester.pumpAndSettle();
        await robot.reorderTask(0, 4); // インデックスを調整
        await tester.pumpAndSettle();
      ...
      // TODO: アニメーション検証のテストは安定性の課題があるため、一時的に無効化
      // アニメーションの検証方法について、より信頼性の高いアプローチを検討する
      /*
      testWidgets('タスク並び替え中のアニメーション検証', (tester) async {
        // テストコード
      });
      */
    });
  ...
  group('TaskList Widget Tests with Controller', () {
    group('Basic Operations', () {
      testWidgets('タスク追加・削除ができる', (tester) async {
        await tester.pumpMyApp(useController: true);
        final robot = TaskListRobot(tester);
